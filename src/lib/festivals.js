...
export async function getFestivalById(id, locale = "ko") {
  ...
  // 비한국어: ① TourAPI 공식 번역(지원 8개 언어) 우선 →
  //           ② 공식 번역이 없는 이름·소개문은 Google 번역으로 보충 →
  //           ③ 그래도 없으면 한국어 원문 유지.
  if (locale !== "ko") {
    const tr = LANG_SERVICE[locale] ? await getTranslation(festival.id, locale) : null;
    let name = tr?.title;
    let description = tr?.overview;

    if (!name || !description) {
      // 소개문 공식 번역이 없으면 한국어 소개문을 확보해 Google 번역
      const koOverview = description
        ? null
        : (await safeOverview(festival.id, apiKey)) || festival.description;
      const [gName, gDesc] = await Promise.all([
        name ? Promise.resolve(name) : translateText(festival.name, locale),
        description ? Promise.resolve(description) : translateText(koOverview, locale),
      ]);
