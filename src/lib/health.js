...
const CHECKS = [
  ...
  },
  {
    ...
    async check() {
      ...
      const r = await ping(`${host}/${process.env.SEOUL_API_KEY}/json/culturalEventInfo/1/1/`);
      if (!r.ok) return { ok: false, detail: `HTTP ${r.status}` };
  ...
  },
  {
    ...
    async check() {
      ...
      const r = await ping(`${base}?serviceKey=${process.env.TOUR_API_KEY}&from=20260101&to=20261231&cPage=1&rows=1&sortStdr=1`);
  ...
  },
  {
    ...
    async check() {
      ...
      const r = await ping(`https://www.googleapis.com/youtube/v3/i18nLanguages?part=snippet&key=${process.env.YOUTUBE_API_KEY}`);
